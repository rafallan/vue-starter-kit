...
<template>
    ...
    <div class="max-w-2xl mx-auto py-8 px-4">
        <div class="mb-6">
            <Link href="/tarefas" class="text-sm text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200">
                &larr; Voltar para listagem
            </Link>
            <h1 class="text-2xl font-semibold mt-2">Nova Tarefa</h1>
        </div>

        <form @submit.prevent="submit" class="space-y-6 bg-white dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            <div class="space-y-2">
                <Label for="titulo">Título</Label>
                <Input id="titulo" v-model="form.titulo" placeholder="Digite o título da tarefa" :class="{ 'border-red-500': form.errors.titulo }" />
            ...
            <div class="flex justify-end gap-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
                <Link href="/tarefas" class="px-4 py-2 text-sm font-medium text-zinc-700 bg-white border border-zinc-300 rounded-md hover:bg-zinc-50 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-700 dark:hover:bg-zinc-700">
                    Cancelar
                </Link>
